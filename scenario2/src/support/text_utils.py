...
def convert_testrail_tables_to_markdown(text):
    ...
    # --- Detect TestRail table blocks ---
    lines = text.split("\n")
    output = []
    table_buffer = []

    def flush_table():
        """Convert buffered TestRail table lines into Markdown format."""
        if not table_buffer:
            return []

        markdown_lines = []

        # Parse each line: TestRail tables use || or ||| as delimiters
        parsed = []
        for row in table_buffer:
            # Remove leading/trailing whitespace, then split by "|"
            cells = [c.strip() for c in row.strip("|").split("|")]
            # Remove empty columns caused by leading "||"
    ...
    # --- Scan the input and detect tables ---
    for line in lines:
